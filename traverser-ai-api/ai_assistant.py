...
class AIAssistant:
    ...
    def _build_prompt(self, xml_context: str, previous_actions: List[str], available_actions: List[str]) -> str:
        """Builds the detailed prompt for the AI model, requesting bounding boxes."""
        action_descriptions = {
            "click": "Click an interactive element (button, link, item). Specify target.",
            "input": "Input text into a text field. Specify target and text.",
        ...
        # *** Updated Prompt requesting Bounding Box ***
        prompt = f"""
        You are an expert Android application tester exploring an app using Appium.
        ...
        2. XML Layout: Provided below. Use this to understand structure and attributes.
        3. Previous Actions on this Screen State: The following actions (described by their target) have already been attempted:
        {history_str}
        ...
        - "target_description": (string) Brief text description of the target (e.g., "button with text 'Login'", "username input field"). Required for 'click' and 'input', null otherwise.
        - "target_bounding_box": (object | null) If action is 'click' or 'input', provide the **normalized** bounding box of the target element. Use the format {{"top_left": [x, y], "bottom_right": [x, y]}}, where x and y are floats between 0.0 and 1.0. Provide this if visually identifiable. Null otherwise or for scroll/back.
        ...
        EXAMPLE (Click Login Button with BBox):
        ```json
        {{
          "action": "click",
          "target_description": "The 'Login' button",
          "target_bounding_box": {{"top_left": [0.4, 0.6], "bottom_right": [0.6, 0.68]}},
          "input_text": null,
          "reasoning": "The login button is the primary action to proceed."
        }}
        ```
        EXAMPLE (Input Email with BBox):
        ```json
        {{
          "action": "input",
          "target_description": "Email address input field",
          "target_bounding_box": {{"top_left": [0.1, 0.3], "bottom_right": [0.9, 0.38]}},
          "input_text": "test@example.com",
          "reasoning": "Need to input email before logging in."
        }}
        ```
