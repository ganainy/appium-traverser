...
from PySide6.QtCore import Qt, QProcess, QTimer, QIODevice, QThread
from PySide6.QtCore import Signal, Slot as slot  # Import Slot as slot to avoid decorator conflicts
from PySide6.QtGui import QPixmap, QColor, QTextCursor, QDesktopServices, QIcon, QPainter, QFont

import requests

from config import Config  # Use the new Config class
...
class CrawlerControllerWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self._config_module_dir_path = self.api_dir
    ...
    def _create_config_inputs(self, layout: QFormLayout):
        ...
        try:
            self.config_widgets['DEFAULT_MODEL_TYPE'].addItems(list(self.config.GEMINI_MODELS.keys()))
        ...
        self.config_widgets['ENABLE_MOBSF_ANALYSIS'] = QCheckBox()
        label_enable_mobsf = QLabel("Enable MobSF Analysis: ")
        label_enable_mobsf.setToolTip("Enable static analysis of the app using MobSF")
        mobsf_layout.addRow(label_enable_mobsf, self.config_widgets['ENABLE_MOBSF_ANALYSIS'])
        
        self.config_widgets['MOBSF_API_URL'] = QLineEdit()
        self.config_widgets['MOBSF_API_URL'].setPlaceholderText("http://localhost:8000/api/v1")
        label_mobsf_api_url = QLabel("MobSF API URL: ")
        label_mobsf_api_url.setToolTip("URL of the MobSF API (e.g., http://localhost:8000/api/v1)")
        mobsf_layout.addRow(label_mobsf_api_url, self.config_widgets['MOBSF_API_URL'])
        
        self.config_widgets['MOBSF_API_KEY'] = QLineEdit()
        self.config_widgets['MOBSF_API_KEY'].setPlaceholderText("Your MobSF API Key")
        self.config_widgets['MOBSF_API_KEY'].setEchoMode(QLineEdit.EchoMode.Password)  # Hide the API key
        label_mobsf_api_key = QLabel("MobSF API Key: ")
        label_mobsf_api_key.setToolTip("API Key for authenticating with MobSF (find in MobSF web interface settings)")
        mobsf_layout.addRow(label_mobsf_api_key, self.config_widgets['MOBSF_API_KEY'])
        
        # Add a button to test MobSF connection
        self.test_mobsf_conn_btn = QPushButton("Test MobSF Connection")
        self.test_mobsf_conn_btn.clicked.connect(self.test_mobsf_connection)
        mobsf_layout.addRow(self.test_mobsf_conn_btn)
        
        # Add a button to run MobSF analysis directly
        self.run_mobsf_analysis_btn = QPushButton("Run MobSF Analysis")
        self.run_mobsf_analysis_btn.clicked.connect(self.run_mobsf_analysis)
        mobsf_layout.addRow(self.run_mobsf_analysis_btn)
    ...
    def log_message(self, message: str, color: str = 'black'):
        ...
        if not hasattr(self, 'log_output') or not self.log_output:
            logging.warning(f"Log output not available. Message: {message}")
            return
        ...
        if app and app.thread() != QThread.currentThread():
            ...
            # For now, just log to console. A more robust solution would use signals/slots.
    ...
    def test_mobsf_connection(self):
        ...
        self.log_message("Testing MobSF connection...")
        api_url = self.config_widgets['MOBSF_API_URL'].text().strip()
        api_key = self.config_widgets['MOBSF_API_KEY'].text().strip()

        if not api_url or not api_key:
            self.log_message("ERROR: MobSF API URL and Key are required.", 'red')
            return
        ...
        try:
            # This should be a GET request according to documentation
            response = requests.get(test_url, headers=headers, timeout=10)
            
            if response.status_code == 200:
                self.log_message("MobSF connection successful!", 'green')
                try:
                    data = response.json()
                    if 'content' in data and isinstance(data['content'], list):
                        self.log_message(f"Successfully fetched {len(data['content'])} recent scans.")
                    else:
                        self.log_message("Received a successful response, but the format was unexpected.")
                except ValueError:
                    self.log_message("Connection successful, but failed to parse JSON response.", 'orange')
                return

            self.log_message(f"ERROR: MobSF connection test failed with status code: {response.status_code}", 'red')
            try:
                # Try to parse the response as text, as it might be HTML
                self.log_message(f"Response: \n{response.text}", 'red')
            except Exception:
                self.log_message("Could not decode the response from the server.", 'red')

        except requests.RequestException as e:
            self.log_message(f"ERROR: MobSF connection test failed: {e}", 'red')

        self.log_message(f"\nAPI URL used: {test_url}", 'blue')
        self.log_message("Important Tips:", 'blue')
        self.log_message("1. Make sure MobSF server is running.", 'blue')
        self.log_message("2. Verify the API URL format - should be 'http://<host>:<port>/api/v1'.", 'blue')
        self.log_message("3. Ensure you're using the correct API key from your MobSF instance.", 'blue')
        self.log_message("4. Check the MobSF API documentation for valid endpoints.", 'blue')
        self.log_message("5. You can find your API key in the MobSF web interface under 'Settings'.", 'blue')

    @slot(int, QProcess.ExitStatus)
    def run_mobsf_analysis(self):
        """Run MobSF analysis for the currently selected app."""
        # Check if an app is selected
        app_package = self.config_widgets['APP_PACKAGE'].text()
        if not app_package:
            self.log_output.append("Error: No app selected. Please select an app before running MobSF analysis.")
            return
            
        # Check if MobSF settings are configured
        api_url = self.config_widgets['MOBSF_API_URL'].text()
        api_key = self.config_widgets['MOBSF_API_KEY'].text()
        
        if not api_url:
            self.log_output.append("Error: MobSF API URL must be configured before running analysis.")
            return
            
        # Security Check: If API key is empty, prompt user to add it to .env file
        if not api_key:
            self.log_output.append("Error: MobSF API Key is not set. Please add your API key in the .env file or in the UI.")
            self.log_output.append("For security, it's recommended to set MOBSF_API_KEY in your .env file rather than in the UI.")
            return
...
"""
            ...
            self.log_output.append(f"Starting MobSF analysis for {app_package}...")
            self.run_mobsf_analysis_btn.setEnabled(False)
            
            # Run the script
            python_executable = sys.executable
            # Use -u for unbuffered stdio to ensure real-time logs
            try:
                from PySide6.QtCore import QProcessEnvironment
                env = QProcessEnvironment.systemEnvironment()
                env.insert("PYTHONUNBUFFERED", "1")
                env.insert("PYTHONIOENCODING", "utf-8")
                self.mobsf_analysis_process.setProcessEnvironment(env)
            except Exception:
                pass
            self.mobsf_analysis_process.start(python_executable, ["-u", script_path])
    ...
    def _on_mobsf_analysis_finished(self, exit_code: int, exit_status: QProcess.ExitStatus):
        ...
        if exit_code == 0 and exit_status == QProcess.ExitStatus.NormalExit:
            self.log_output.append("MobSF analysis completed successfully.")
        else:
            self.log_output.append(f"MobSF analysis failed with exit code {exit_code}.")
            
        self.run_mobsf_analysis_btn.setEnabled(True)
