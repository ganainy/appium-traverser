...
class Config:
        ...
        try:
            ...
            for key, value in defaults_vars.items():
                ...
                if hasattr(self, key):
                    ...
                    if expected_attr is not None and not isinstance(value, type(expected_attr)) and type(expected_attr) not in [Union, List, Dict, Any]:
                         # Check if the target type is a Union (like Optional)
                        is_union = getattr(type(expected_attr), "__origin__", None) is Union
                        if not is_union : # only warn if not a Union type that could accept None or other types
        ...
        if perform_type_conversion:
            ...
            if target_type_hinted is not None:
                ...
                if origin_type is Union:
                    ...
                    if len(non_none_args) == 1: actual_target_type = non_none_args[0]
                elif origin_type is list and len(args_type) == 1: actual_target_type = list
                elif origin_type is dict and len(args_type) == 2: actual_target_type = dict
                elif origin_type is None: actual_target_type = target_type_hinted

                if actual_target_type and not isinstance(new_value, actual_target_type):
                    try:
                        if actual_target_type == bool: converted_value = str(new_value).lower() in ['true', '1', 'yes', 'on']
                        elif actual_target_type == int: converted_value = int(new_value)
                        elif actual_target_type == float: converted_value = float(new_value)
    ...
    def _get_user_savable_config(self) -> Dict[str, Any]:
        savable_keys = [
            ...
            "LOG_LEVEL", "DB_CONNECT_TIMEOUT", "DB_BUSY_TIMEOUT",
            ...
            "OPENROUTER_REFRESH_BTN", "OPENROUTER_SHOW_FREE_ONLY",
...
ALLOWED_EXTERNAL_PACKAGES = [
    ...
    "com.google.android.permissioncontroller", "org.mozilla.firefox",
