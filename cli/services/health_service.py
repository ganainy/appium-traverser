...
class HealthCheckService:
    ...
    def check_all_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        ai_provider = getattr(self.config, KEYS.CONFIG_AI_PROVIDER, KEYS.AI_PROVIDER_GEMINI).lower()
        ...
        # Check target app
        app_package = getattr(self.config, KEYS.CONFIG_APP_PACKAGE, None)
        if app_package:
        ...
        try:
            ...
            try:
                ...
                if response.status_code == KEYS.HTTP_CODE_OK:
                    ...
                    uptime_ms = ready_data.get(KEYS.JSON_KEY_UPTIME_MS, 0)
                    registered_tools = ready_data.get(KEYS.JSON_KEY_REGISTERED_TOOLS, 0)
                    active_invocations = ready_data.get(KEYS.JSON_KEY_ACTIVE_INVOCATIONS, 0)
                    return {
    ...
    def check_api_keys_and_env(self) -> Tuple[List[str], List[str]]:
        ...
        if ai_provider == KEYS.AI_PROVIDER_GEMINI:
            if not getattr(self.config, KEYS.CONFIG_GEMINI_API_KEY, None):
                issues.append(MSG.PRECHECK_STATUS_MESSAGE_MISSING_KEY.format(key="Gemini API key", env=KEYS.CONFIG_GEMINI_API_KEY))
        elif ai_provider == KEYS.AI_PROVIDER_OPENROUTER:
            if not getattr(self.config, KEYS.CONFIG_OPENROUTER_API_KEY, None):
                issues.append(MSG.PRECHECK_STATUS_MESSAGE_MISSING_KEY.format(key="OpenRouter API key", env=KEYS.CONFIG_OPENROUTER_API_KEY))
        elif ai_provider == KEYS.AI_PROVIDER_OLLAMA:
            if not getattr(self.config, KEYS.CONFIG_OLLAMA_BASE_URL, None):
                warnings.append(MSG.PRECHECK_STATUS_MESSAGE_OLLAMA_DEFAULT)
        
        # Check traffic capture API key
        if getattr(self.config, KEYS.CONFIG_ENABLE_TRAFFIC_CAPTURE, False):
            if not getattr(self.config, KEYS.CONFIG_PCAPDROID_API_KEY, None):
                issues.append(MSG.PRECHECK_STATUS_MESSAGE_MISSING_KEY.format(key="PCAPDroid API key", env=KEYS.CONFIG_PCAPDROID_API_KEY))
        
        # Check MobSF API key
        if getattr(self.config, KEYS.CONFIG_ENABLE_MOBSF_ANALYSIS, False):
            if not getattr(self.config, KEYS.CONFIG_MOBSF_API_KEY, None):
                issues.append(MSG.PRECHECK_STATUS_MESSAGE_MISSING_KEY.format(key="MobSF API key", env=KEYS.CONFIG_MOBSF_API_KEY))
