...
1. **XPath Exact Match**: `//element[@text='Exact Text']`
2. **ID**: Find by resource-id
3. **Accessibility ID**: Find by content-desc
4. **XPath Contains Match**: `//element[contains(@text, 'Partial')]`
...
- `steps`: Individual crawl actions
- `ai_interactions`: AI decision records
...
class Config:
    ...
    # Crawling Behavior
    CRAWL_MODE: str = "time"  # "time" or "steps"
...
```python
# CLI Updates
cli_controller.py --set-config MAX_CRAWL_STEPS=50
cli_controller.py --save-config

# Programmatic Updates
config.update_setting_and_save('MAX_CRAWL_STEPS', 50)
```
